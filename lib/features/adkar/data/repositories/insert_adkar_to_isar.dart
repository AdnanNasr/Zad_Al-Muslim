...
Future<void> insertAdkarToIsar() async {
  ...
  try {
    ...
    if (existingCount > 0) {
      AppLogger.logger.w("⚠️ قاعدة البيانات تحتوي مسبقاً على $existingCount ذكر. لن يتم الإدخال مجدداً.");
      return;
    }

    AppLogger.logger.i("📂 جاري تحميل ملف hisn_almuslim.json من الـ assets...");
    final content = await rootBundle.loadString("assets/json/hisn_almuslim.json");
    ...
    AppLogger.logger.i("📊 تم العثور على ${adkarList.length} تصنيف للأذكار. جاري الإدخال...");
    ...
    AppLogger.logger.i("✅ تم إدخال الأذكار إلى قاعدة البيانات بنجاح! إجمالي الأنواع المحفوظة: $finalCount");
  } catch (e, stackTrace) {
    AppLogger.logger.e(
