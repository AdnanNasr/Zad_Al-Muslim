...
class _PrayTimePageState extends ConsumerState<PrayTimePage>
  ...
  Future<void> _recheckLocationOnResume() async {
    ...
    if (networkState == NetworkInfoState.connected) {
      ...
      // Set loading status before starting the fetch
  ...
  Widget build(BuildContext context) {
    ...
    // fetch the location automatically.
    ...
    // also monitor location status: when permissions are granted, try to fetch it.
      ...
      child: Scaffold(
        ...
        body: SafeArea(
          child: RefreshIndicator(
            onRefresh: () => ref.read(userAddressProvider.notifier).refresh(),
  ...
  Widget _buildErrorState({
    ...
    // 2. حالة عدم وجود اتصال بالإنترنت (الأولوية القصوى للعرض)
    if (networkState == NetworkInfoState.notConnected) {
      return _buildNoInternetWidget();
    }

    // 3. التحقق من وجود "مانع" فعلي (مثل تفعيل الخدمة أو الأذونات)
    // قبل الانتقال لحالة التحميل الرمزي
    ...
    // 4. إذا كانت الحالة فارغة تماماً (أول دخول والشبكة متصلة ولكن لم نبدأ الفحص بعد)
    if (status.isEmpty) {
      return Center(
        child: Column(
          mainAxisAlignment: MainAxisAlignment.center,
    ...
    // 6. الحالة العامة لأي خطأ آخر غير متوقع
    return _buildActionErrorUI(context, status, technicalError: error);
  }

  // ويدجت موحد لعرض الأخطاء التي تتطلب إجراء من المستخدم (مثل تفعيل GPS أو الأذونات)
  ...
  Widget _buildActionButtons(
    ...
    // أزرار الأكشن لتبسيط الكود الأساسي
